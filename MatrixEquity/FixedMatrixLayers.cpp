...
FixedMatrixLayers::FixedMatrixLayers(vector<string> inpMatrixCards, ProbToWinWithCard* PTWWC, 
									CardsCombos* CardCombos, EqSummTables* eqTables, setsOfHittedCells* SetsOfHittedCells) {
	matrixCards = inpMatrixCards;
	itsPTWWC = PTWWC;
	itsCardCombos = CardCombos;
	itsEqTables = eqTables;
	itsSetsOfHittedCells = SetsOfHittedCells;
}
...
void FixedMatrixLayers::EVAndAntiEVFromXHits(MatrixStatus& MS, FLayer* currLayer, int hitsAmount,
	...
	if (hitsAmount == 0) {
		...
		vector<vector<int>>* setsOfHittedCells = &itsSetsOfHittedCells->setsOfChildren[MS.closedCellsAsInt][hitsAmount];
		for (vector<int>& hittedCells : *setsOfHittedCells) {

			//for speed test
			float MfoldEV = currLayer->EV[MS.closedCellsAsInt];
			MatrixStatus Mpush = MS;
			for (int cellInd : hittedCells) {
				Mpush.closeC(cellInd);
			}
			float MPushAndWinEV = currLayer->EV[Mpush.closedCellsAsInt];
			float MatrixEVdiff = MfoldEV - MPushAndWinEV;
			//насправді при створенні таблиць треба не на мінусове ЕВприАІ дивитись,
			//а на мінусове ЕВ при АІ, розділене на шанс виграти роздачу у випадку олл-іна !!!
			//тобто ми ризикуємо фдучи олл-ін лише у випадку, коли це перевищуватиме
			//різницю у вартості матриці пуша_і_виграша і матриці фолда
			//але поки для тесту швидкосту запущу на неправильних таблицях щось

			//find index of right EV_edge;
			int indexForEdge = 5; //for test;
			int indCardCombos = 10; //it's card 'Q'
			tableEVandProbs& table = itsEqTables->vTableData[indCardCombos];
			float probWinForEdge = table.probToWinSd[indexForEdge]; //if AI only with combos that can go AI
			float antiEVforEdge = table.antiEVforThisEdge[indexForEdge]; //if AI only with combos that can go AI
			
			//get probability to have right 2 fixed cells in hero hand (P1);
			float prob_P1 = p1_probToHitXCells(MS, hitsAmount);

			float EVifHitted = (probWinForEdge * MPushAndWinEV + (1 - probWinForEdge) * MfoldEV);
			float antiEVifHitted = antiEVforEdge;
			collectedEV += prob_P1 * EVifHitted;
			collectedAntiEV += prob_P1 * antiEVifHitted;
		}
		EV = collectedEV;
